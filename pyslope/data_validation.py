...
def assert_positive_number(n, name):
    ...
    if not isinstance(n, (int, float)):
        raise ValueError(
            f"The value for '{name}' must be an int or float, "
            f"not {type(n)}."
        )
    if n < 0:
...
def assert_strictly_positive_number(n, name):
    ...
    if not isinstance(n, (int, float)):
        raise ValueError(
            f"The value for '{name}' should be an int or float, not a "
            f"{type(n).__name__}."
        )
    if n < 0:
        raise ValueError(f"The value for '{name}' should be > 0, not {n}")
...
def assert_number(n, name):
    ...
    if type(n) not in [int, float]:
        raise ValueError(
            f"The value for '{name}' should be an integer or a float, "
            f"not a {type(n)}."
        )
...
def assert_range(n, name, low, high, not_low=False, not_high=False):
    ...
    if not isinstance(low, (int, float)) or not isinstance(high, (int, float)):
        raise ValueError(
            f"The value for the ranges low and high should be an integer "
            f"or a float not a {type(low)} and {type(high)}."
        )
    if n < low:
        raise ValueError(
    ...
    if n > high:
        raise ValueError(
            f"The value for '{name}' should be less than {high}, not {n}."
        )

    if not_low and n == low:
        raise ValueError(
            f"The value for '{name}' should not be equal to the lower bound "
            f"({low})."
        )

    if not_high and n == high:
        raise ValueError(
            f"The value for '{name}' should not be equal to the upper bound "
            f"({high})."
        )
...
def assert_length(li, num, name):
    ...
    if len(li) != num:
        raise ValueError(
            f"The length of '{name}' should be {num} not {len(li)}."
        )
